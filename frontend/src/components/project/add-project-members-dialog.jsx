...
export function AddProjectMembersDialog({
    ...
    const { members: workspaceMembers, fetchWorkspaceMembers, loading: workspaceLoading } = useWorkspace();
    ...
    const availableMembers = workspaceMembers?.filter(
        m => !existingMemberIds.includes(m.user?._id || m.user)
    ) || [];
    ...
    const handleAddMembers = async () => {
        ...
        for (const memberId of memberIds) {
            try {
                await addProjectMember(project._id, memberId, selectedMembers[memberId]);
                successCount++;
            } catch (error) {
                errorCount++;
                    ...
                    >
                        {adding ? (
                            ...
                            <>
                                ...
                                Add {selectedCount > 0 ? `${selectedCount} Member${selectedCount > 1 ? 's' : ''}` : 'Members'}
                            </>
                        )}
                    </Button>
                </DialogFooter>
